...
function validateLayer(layer: unknown, index: number): string | null {
  ...
  if (typeof layer.decalKey !== 'string') return `${path}.decalKey must be a string`
...
export function validateDecalJson(input: string): ValidationResult {
  ...
  if (layers.length > MAX_LAYERS) {
    return { ok: false, error: `Too many layers: ${layers.length} exceeds the maximum of ${MAX_LAYERS}.` }
  }
