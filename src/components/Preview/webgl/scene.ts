...
export function initScene(
  ...
  let state: SceneState = { layers: [], selectedLayerIndex: null, showMode: 'all' }
  let dirty = true
  ...
  function unproject(nx: number, ny: number, nz: number, m: Float32Array): Vec3 {
  ...
  function rayToSegmentDist(O: Vec3, D: Vec3, A: Vec3, B: Vec3): number {
    ...
    const px = O[0] + t * D[0],  qx = A[0] + s * V[0]
    const py = O[1] + t * D[1],  qy = A[1] + s * V[1]
    const pz = O[2] + t * D[2],  qz = A[2] + s * V[2]
