...
export function buildArrow(origin: Vec3, tip: Vec3, dir: Vec3): ArrowVerts {
  ...
  for (let i = 0; i < ARROW_CONE_SEGMENTS; i++) {
    ...
    const p0 = add(
      coneBase,
      add(
        scale(right, Math.cos(a0) * ARROW_CONE_RADIUS),
        scale(up2, Math.sin(a0) * ARROW_CONE_RADIUS),
      ),
    );
    const p1 = add(
      coneBase,
      add(
        scale(right, Math.cos(a1) * ARROW_CONE_RADIUS),
        scale(up2, Math.sin(a1) * ARROW_CONE_RADIUS),
      ),
    );
